        ...
        for level_idx, stride in enumerate(strides):
            ...
            for batch_idx in range(batch_size):
                ...
                if num_pos_level > 0:
                    ...
                    if cls_counts[1] < 3:
                        class_weights = torch.tensor([1.0, 7.0], device=device)  # 从30.0降至7.0
                    else:
                        cls_ratio = max(1.0, cls_counts[0].float() / max(cls_counts[1].float(), 1))
                        class_weights = torch.tensor([1.0, min(5.0, max(3.0, cls_ratio))], device=device)  # 显著降低权重上限和倍数
                    
                    print(f"[WEIGHT] Class distribution: {cls_counts.tolist()}, weights: {class_weights.tolist()}")
                else:
                    class_weights = torch.tensor([1.0, 10.0], device=device)  # 从50.0降至10.0
                
                gamma = 2.0 # 从4.0降低到2.0，减少对难分样本的过度关注
                alpha = 0.25
                ...
                if len(neg_inds) > 0 and len(pos_inds) > 0:
                    ...
                    #困难负样本,模型容易误判为正样本的负样本
                    num_hard_negs = min(len(neg_inds), int(len(pos_inds) * 1.5))
        ...
        try:
            ...
            for batch_idx in range(batch_size):
                ...
                for level_idx, (cls_score, reg_pred, centerness) in enumerate(zip(cls_scores, reg_preds, centernesses)):
                    ...
                    cls_boost[:, 0] = 1.1  # 为类别0提供轻微提升
                    cls_boost[:, 1] = 1.3  # 为类别1提供适度提升（从1.2调整）
...
# class FCOSHead(nn.Module):
#     def __init__(self, num_classes=2, in_channels=[256, 256, 256], strides=[4, 8, 16], use_gn=True, 
#                  feat_channels=256, init_prior=0.01, scale_exp_init=1.0, center_sampling=True, 
#                  center_radius=4.0):
#         super().__init__()
#         self.num_classes = num_classes
...
#         self.debug_logger = None
#         self.class_stats = None  # 新增: 用于跟踪类别统计
        
#         # 新增: 记录每个层级的正样本数
#         self.level_pos_samples = []
...
#             for j in range(4):
#                 layer_in_c = in_c if j == 0 else feat_channels  # 修改: 使用当前层的输入通道数
...
#         # 新增: 正样本统计
#         self.pos_samples_by_class = torch.zeros(self.num_classes)
...
#                         bias_data = torch.zeros_like(m.bias.data)
#                         bias_data[0] = -bias_value
#                         bias_data[1] = 1.0  # 新增: 为类别1提供更高的初始偏置
#                         m.bias.data.copy_(bias_data)
#                     else:
#                         nn.init.constant_(m.bias, -bias_value)
#                 else:
...
#         num_pos = 0  # 累计所有层级的正样本总数
#         total_locations = 0  # 累计所有层级的特征点总数
...
#         # 新增: 检查类别1是否存在于targets中
#         verbose = self.debug_logger is None or (hasattr(self.debug_logger, 'iteration') and self.debug_logger.iteration % 200 == 0)
#         if verbose:
...
#                         all_batch_cls1_count += cls1_mask.sum().item()
...
#             locations = locations.view(-1, 2)
#             level_locations_count = locations.shape[0] * batch_size
#             total_locations += level_locations_count
            
#             # 新增: 记录当前层级的位置数
#             self.level_locations.append(level_locations_count)
            
#             level_pos = 0  # 当前层级的正样本总数
...
#                     if cls1_mask.any():
#                         # 对所有类别1目标增加半径
#                         radius_x[cls1_mask] = self.center_radius * 5.0  # 显著增加类别1的中心采样半径
#                         radius_y[cls1_mask] = self.center_radius * 5.0
                        
#                         # 对小目标进一步增加半径
#                         obj_area = (gt_x2 - gt_x1) * (gt_y2 - gt_y1)
#                         small_obj_mask = cls1_mask & (obj_area < 32*32)
#                         if small_obj_mask.any():
#                             radius_x[small_obj_mask] = self.center_radius * 7.0  # 对小类别1目标使用更大半径
#                             radius_y[small_obj_mask] = self.center_radius * 7.0
                            
#                             if verbose:
#                                 print(f"[CENTER-SAMPLING] 小类别1目标数量: {small_obj_mask.sum().item()}, "
#                                       f"半径系数: {7.0}")
...
#                     # 保存原始的框内掩码，用于回退
#                     original_inside_gt_mask = inside_gt_mask.clone()
                    
#                     # 应用中心采样约束
#                     inside_gt_mask = inside_gt_mask & inside_center
                    
#                     # 修改: 检查类别1的中心采样效果
#                     if verbose and cls1_mask.any():
#                         cls1_inds = torch.nonzero(cls1_mask).squeeze(1)
#                         print(f"[CENTER-SAMPLING] 找到{len(cls1_inds)}个类别1的目标")
                        
#                         for idx in cls1_inds:
#                             # 获取该目标的中心采样区域内的点数
#                             center_x, center_y = gt_centers[idx]
#                             r_x, r_y = radius_x[idx], radius_y[idx]
                            
#                             # 计算点与中心的距离
#                             dist_x = (locations[:, 0] - center_x).abs()
#                             dist_y = (locations[:, 1] - center_y).abs()
                            
#                             # 统计在采样区域内的点数
#                             in_center_x_mask = dist_x < r_x
#                             in_center_y_mask = dist_y < r_y
#                             in_center_mask = in_center_x_mask & in_center_y_mask
                            
#                             in_gt_mask = original_inside_gt_mask[:, idx]
#                             combined_mask = in_center_mask & in_gt_mask
                            
#                             print(f"[CENTER-SAMPLING] 类别1目标 #{idx}: 中心=({center_x:.1f},{center_y:.1f}), "
#                                   f"半径=({r_x:.1f},{r_y:.1f}), "
#                                   f"区域内点数={in_center_mask.sum().item()}, "
#                                   f"框内点数={in_gt_mask.sum().item()}, "
#                                   f"有效正样本点数={combined_mask.sum().item()}")
                    
#                     # 修改: 对于类别1，如果中心采样后没有正样本，则回退到框内采样
#                     if cls1_mask.any():
#                         for cls1_idx in torch.nonzero(cls1_mask).squeeze(1):
#                             # 检查该目标是否有正样本
#                             has_pos_samples = inside_gt_mask[:, cls1_idx].any()
                            
#                             # 如果没有正样本，则使用框内所有点
#                             if not has_pos_samples:
#                                 inside_gt_mask[:, cls1_idx] = original_inside_gt_mask[:, cls1_idx]
#                                 if verbose:
#                                     print(f"[CENTER-FALLBACK] 类别1目标 #{cls1_idx} 中心采样后无正样本，使用框内所有点 "
#                                           f"({original_inside_gt_mask[:, cls1_idx].sum().item()}个)")
...
#                 # 新增: 更新类别统计
#                 for cls_id in range(self.num_classes):
#                     cls_count = (gt_cls[target_inds] == cls_id).sum().item()
#                     pos_samples_by_class[cls_id] += cls_count
                    
#                     # 如果有类别统计字典，更新它
#                     if self.class_stats is not None:
#                         if cls_id == 0:
#                             self.class_stats['total_pos_cls0'] += cls_count
#                         elif cls_id == 1:
#                             self.class_stats['total_pos_cls1'] += cls_count
...
#                 # 新增: 为小目标提供更宽松的中心度计算
#                 small_obj_mask = (max_left_right * max_top_bottom) < (32 * 32)  # 小于32x32的目标
...
#                 if small_obj_mask.any():
#                     # 为小目标提供更高的中心度（平方根提升）
#                     centerness_targets[small_obj_mask] = torch.sqrt(centerness_targets[small_obj_mask])
                
#                 # 特殊处理类别1的小目标
#                 cls1_small_mask = small_obj_mask & (gt_cls[target_inds] == 1)
#                 if cls1_small_mask.any():
#                     # 显著提高小行人目标的中心度
#                     centerness_targets[cls1_small_mask] = torch.pow(centerness_targets[cls1_small_mask], 0.3)
...
#                     elif cls_counts[1] < 5:
#                         # 类别1样本很少，使用较高权重
#                         class_weights = torch.tensor([1.0, 12.0], device=device)
#                         print(f"[WEIGHT] Class distribution: {cls_counts.tolist()}, weights: {class_weights.tolist()}")
#                     else:
#                         # 根据实际比例计算权重，但限制最大值
#                         cls_ratio = max(1.0, cls_counts[0].float() / max(cls_counts[1].float(), 1))
#                         weight_cls1 = min(10.0, max(5.0, cls_ratio * 1.5))  # 减小系数
#                         class_weights = torch.tensor([1.0, weight_cls1], device=device)
#                         print(f"[WEIGHT] Class distribution: {cls_counts.tolist()}, weights: {class_weights.tolist()}")
#                 else:
#                     class_weights = torch.tensor([1.0, 8.0], device=device)
                
#                 # 修改: 降低Focal Loss参数以减少对难分样本的过度关注
#                 gamma = 2.0
#                 alpha = 0.25
...
#                         focal_weight[sample_cls1_mask] = focal_weight[sample_cls1_mask] * 1.25  # 从1.5降至1.25
...
#                 # 修改: 结合GIoU和L1损失进行回归
#                 giou_loss = self._calculate_giou_loss(pred_boxes, target_boxes)
#                 l1_loss = F.l1_loss(pred_boxes, target_boxes, reduction='none').sum(dim=-1)
                
#                 # 为类别1提供更大的回归损失权重
#                 reg_loss_weight = torch.ones_like(giou_loss)
#                 if cls1_mask.any():
#                     reg_loss_weight[cls1_mask] = 2.0  # 增加类别1的回归损失权重
                
#                 # 组合GIoU和L1损失
#                 combined_reg_loss = (0.8 * giou_loss + 0.2 * l1_loss) * reg_loss_weight
                
#                 if torch.isnan(combined_reg_loss).any():
#                     print(f"[WARNING] NaN in combined_reg_loss, replacing with zeros")
#                     combined_reg_loss = torch.where(torch.isnan(combined_reg_loss), torch.zeros_like(combined_reg_loss), combined_reg_loss)
                
#                 reg_loss = reg_loss + (combined_reg_loss * centerness_targets).sum()
...
#                     neg_loss_sum = neg_loss.sum(dim=1)
#                     # 修改: 降低困难负样本的比例
#                     num_hard_negs = min(len(neg_inds), int(len(pos_inds) * 1.5))  # 从3倍降至1.5倍
...
#             # 新增: 记录当前层级的正样本数
#             self.level_pos_samples.append(level_pos)
        
#         # 新增: 保存类别正样本统计
#         self.pos_samples_by_class = pos_samples_by_class
...
#         # 新增: 打印每个层级的正样本统计
#         if verbose and len(self.level_pos_samples) > 0:
#             print(f"[LEVEL-STATS] Positive samples by level: {self.level_pos_samples}")
#             print(f"[LEVEL-STATS] Locations by level: {self.level_locations}")
            
#             for i, (pos_count, loc_count) in enumerate(zip(self.level_pos_samples, self.level_locations)):
#                 ratio = pos_count / max(1, loc_count) * 100
#                 print(f"  Level {i} (stride={strides[i]}): {pos_count} pos / {loc_count} locs = {ratio:.4f}%")
...
#         try:
#             # 类别特定的阈值
#             score_thr_cls0 = 0.15  # 为类别0设置较高阈值
#             score_thr_cls1 = 0.05 if score_thr_cls1 is None else score_thr_cls1  # 为类别1设置更低阈值
#             nms_thr_cls0 = nms_thr + 0.05  # 为类别0也提供稍宽松的NMS阈值
#             nms_thr_cls1 = nms_thr + 0.2 if nms_thr_cls1 is None else nms_thr_cls1  # 为类别1提供更宽松的NMS阈值
...
#                     cls_boost = torch.ones_like(cls_sigmoid)
#                     cls_boost[:, 0] = 1.1  # 为类别0提供轻微提升
#                     cls_boost[:, 1] = 1.3  # 为类别1提供适度提升
                    
#                     # 修改: 对于小目标，减少中心度的影响
#                     small_obj_mask = (bboxes[:, 2] - bboxes[:, 0]) * (bboxes[:, 3] - bboxes[:, 1]) < (32 * 32)
#                     ctr_weight = torch.ones_like(ctr_sigmoid)
#                     if small_obj_mask.any():
#                         ctr_weight[small_obj_mask] = 0.8  # 减少中心度权重对小目标的影响
                    
#                     # 最终得分计算
#                     scores = cls_sigmoid * (ctr_sigmoid ** ctr_weight) * cls_boost
...
#             threshold_cls0 = threshold + 0.05 if threshold_cls0 is None else threshold_cls0
#             threshold_cls1 = threshold + 0.2 if threshold_cls1 is None else threshold_cls1
...
#         """返回训练中的类别统计信息"""
#         return self.pos_samples_by_class.clone() if hasattr(self, 'pos_samples_by_class') else None

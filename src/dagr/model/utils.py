...
class mAPCalculator:
    """与官方COCO实现兼容的mAP计算器，跳过没有GT框的图像"""
    def __init__(self, iou_threshold=0.5, num_classes=2):
        self.iou_threshold = iou_threshold
        self.num_classes = num_classes
        self.class_names = {0: 'car', 1: 'person'}  # 类别名称映射
        self.reset()
    ...
    def reset(self):
        ...
        self.num_images = 0
        # 为了与您的代码兼容，保持这些属性
        self.all_detections = []  # 保存所有检测结果的列表
        self.all_scores = []      # 保存所有得分的列表

    def update(self, pred_boxes, pred_scores, pred_labels, gt_boxes, gt_labels):
        """
        更新评估器状态，仅添加至少包含一个GT框的图像
        """
        # 跳过没有GT框的图像 - 与官方COCO实现一致
        if len(gt_boxes) == 0:
            return
        ...
        # 为了兼容性，同时更新all_detections和all_scores
        self.all_detections.append({'boxes': pred_boxes})
        self.all_scores.append(pred_scores)

    def compute_per_class(self, class_id, verbose=False):
        """计算单个类别的AP，使用COCO风格评估"""
        if self.num_images == 0:
            return 0.0

        # 整理所有的检测和GT
        all_detections = []  # [image_idx, pred_idx, score, is_tp]
        class_gt_count = 0   # 这个类别的GT框总数
        ...
        for img_idx in range(self.num_images):
            ...
            # 增加这个类别的GT计数
            class_gt_count += len(gt_class_boxes)

            # 如果没有当前类别的预测，则继续下一张图像
            if len(pred_class_boxes) == 0:
                continue

            # 计算IoU矩阵 (num_preds, num_gts)
            iou_matrix = box_iou(pred_class_boxes, gt_class_boxes)

            # 标记已经匹配的GT框
            gt_matched = torch.zeros(len(gt_class_boxes), dtype=torch.bool)
            ...
            for pred_idx in sorted_indices:
                ...
                if len(gt_class_boxes) == 0:
                    all_detections.append([img_idx, pred_idx.item(), pred_class_scores[pred_idx].item(), 0])
                    continue
                ...
                # 判断是否匹配
                if max_iou >= self.iou_threshold and not gt_matched[max_gt_idx]:
                    # 真阳性
                    all_detections.append([img_idx, pred_idx.item(), pred_class_scores[pred_idx].item(), 1])
        ...
        # 如果没有GT或没有预测，AP为0
        if class_gt_count == 0 or len(all_detections) == 0:
            return 0.0

        # 将检测结果转换为numpy数组并按置信度排序
        all_detections = np.array(all_detections)
        sorted_indices = np.argsort(-all_detections[:, 2])  # 按置信度降序
        all_detections = all_detections[sorted_indices]

        # 计算累积TP和FP
        tp = all_detections[:, 3]  # is_tp列
        fp = 1 - tp
        ...
        if verbose:
            print(f"Class {class_id} ({self.class_names.get(class_id, 'unknown')}):")
            print(f"  - AP: {ap:.6f}")
            print(f"  - GT count: {class_gt_count}")
            print(f"  - Detections: {len(all_detections)}")
            print(f"  - TP: {tp_cumsum[-1] if len(tp_cumsum) > 0 else 0}")
            print(f"  - FP: {fp_cumsum[-1] if len(fp_cumsum) > 0 else 0}")

        return ap

    def compute_per_image(self, image_idx, verbose=False):
        """为单张图像计算AP和详细指标 - 保留原函数以兼容"""
        if image_idx >= self.num_images:
            return {'AP': 0.0, 'details': 'Invalid image index'}
    ...
    def compute(self, verbose=True):
        ...
        if self.num_images == 0:
            # 与官方保持一致，没有评估图像返回0
            return {'mAP': 0.0, 'details': 'No valid images with GT boxes'}

        # 计算每个类别的AP
        ap_per_class = []
        for class_id in range(self.num_classes):
            ap = self.compute_per_class(class_id, verbose=False)
        ...
        if verbose:
            ...
            for class_id, ap in enumerate(ap_per_class):
                print(f"[mAP] Class {class_id} ({self.class_names.get(class_id, 'unknown')}): AP = {ap:.6f}")
            
            # 输出评估信息
            print(f"[mAP] Evaluated on {self.num_images} images with GT boxes")
            
            # 如果mAP为0，给出一些简单的诊断信息
            if mAP == 0:
                print("\n[mAP] mAP is ZERO - possible reasons:")
                # 统计预测数量
                total_preds = sum(len(preds) for preds in self.all_pred_boxes)
                if total_preds == 0:
                    ...
                    for img_idx in range(self.num_images):
                        ...
                        if len(gt_boxes) > 0 and len(pred_boxes) > 0:
                            ...
                            max_ious = iou.max(dim=1)[0]
                            low_iou_count += (max_ious < self.iou_threshold).sum().item()
                            total_iou_pairs += len(max_ious)
                    
                    if total_iou_pairs > 0:
                        low_iou_ratio = low_iou_count / total_iou_pairs
                        print(f"  - {low_iou_ratio:.2%} of predictions have IoU < {self.iou_threshold} with any GT box")
                    
                    # 检查置信度问题
                    all_scores = []
                    for scores in self.all_pred_scores:
                        if len(scores) > 0:
                            all_scores.append(scores)
                    
                    if all_scores:
                        all_scores = torch.cat(all_scores)
                        print(f"  - Confidence scores: min={all_scores.min().item():.4f}, max={all_scores.max().item():.4f}, mean={all_scores.mean().item():.4f}")
        ...
        # 添加每个类别的AP
        for cls_id, ap in enumerate(ap_per_class):
            metrics[f'AP_{cls_id}'] = ap
...
#             print(f"    Stats: mean={layer_stats['mean']:.4f}, std={layer_stats['std']:.4f}, min={layer_stats['min']:.4f}, max={layer_stats['max']:.4f}")
#             if 'grad_mean' in layer_stats:
#                 print(f"    Grad stats: mean={layer_stats['grad_mean']:.4f}, std={layer_stats['grad_std']:.4f}, min={layer_stats['grad_min']:.4f}, max={layer_stats['grad_max']:.4>
#     return stats, problematic_layers
...
#                 # 分析置信度分布
#                 all_pred_scores = torch.cat([scores for scores in self.all_pred_scores if len(scores) > 0], dim=0) if any(len(scores) > 0 for scores in self.all_pred_scores) else>

...
class Net(torch.nn.Module):
    ...
    def forward(self, data: Data, reset=True):
        if self.use_image:
            ...
            print(f"[DEBUG] image_outputs shapes: {[out.shape for out in image_outputs]}")
            print(f"[DEBUG] image_feat shapes: {[feat.shape for feat in image_feat]}")
            
        if hasattr(data, 'reset'):
            reset = data.reset
        
        # 在events_to_graph前添加事件数据格式检查
        print("\n===== EVENT DATA FORMAT INSPECTION =====")
        print(f"Data type: {type(data)}")
        print(f"Data attributes: {data.__dict__.keys()}")
        
        # 检查常见属性
        if hasattr(data, 'x'):
            print(f"data.x shape: {data.x.shape}, dtype: {data.x.dtype}")
            print(f"data.x first few values: {data.x[:5]}")
        
        if hasattr(data, 'pos'):
            print(f"data.pos shape: {data.pos.shape}, dtype: {data.pos.dtype}")
            print(f"data.pos first few values: {data.pos[:5]}")
        
        if hasattr(data, 'batch'):
            print(f"data.batch shape: {data.batch.shape}, dtype: {data.batch.dtype}")
            print(f"data.batch unique values: {torch.unique(data.batch)}")
        
        if hasattr(data, 'width'):
            print(f"data.width: {data.width}")
        ...
        if hasattr(data, 'events'):
            print(f"data.events type: {type(data.events)}")
            print(f"data.events shape/length: {data.events.shape if hasattr(data.events, 'shape') else len(data.events)}")
            if hasattr(data.events, 'shape'):
                print(f"data.events first few: {data.events[:5]}")
            elif hasattr(data.events, '__getitem__'):
                print(f"data.events first few: {data.events[:5]}")
        print("======================================\n")
        
        data = self.events_to_graph(data, reset=reset)
        
        # events_to_graph处理后再次检查数据
        print("\n===== AFTER events_to_graph =====")
        print(f"Data type: {type(data)}")
        print(f"Data attributes: {data.__dict__.keys()}")

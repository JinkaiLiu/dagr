...
class FCOSHead(nn.Module):
    ...
    def loss(self, cls_scores, reg_preds, centernesses, targets):
        ...
        for b in range(B):
            ...
            cls_target = torch.zeros_like(all_cls_scores[b])
            cls_target[pos_mask, matched_cls] = 1.0
            loss_cls = self.focal_loss(all_cls_scores[b], cls_target, alpha=0.25, gamma=2.0) / max(num_pos, 1)
            ...
            reg_pred = all_reg_preds[b][pos_mask]
            ...
            pred_ctr = all_centernesses[b][pos_mask].squeeze()
    ...
    def decode_outputs(self, cls_scores, reg_preds, centernesses):
        ...
        for b in range(B):
            ...
            for i, (cls, reg, ctr) in enumerate(zip(cls_scores, reg_preds, centernesses)):
                cls = torch.sigmoid(cls[b])
                reg = reg[b]
                ctr = torch.sigmoid(ctr[b][0])
            ...
            if boxes_all:
                ...
                onehot[torch.arange(len(labels_all)), labels_all] = 1.0
